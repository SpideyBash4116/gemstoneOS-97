...
export default function MineralClicker() {
  ...
  // Load game state
  useEffect(() => {
    const savedGems = localStorage.getItem("gemstoneOS_clicker_gems");
    const savedUpgrades = localStorage.getItem("gemstoneOS_clicker_upgrades");
    
    if (savedGems) {
      setTotalGems(parseFloat(savedGems));
    }
    if (savedUpgrades) {
      try {
